...
처음엔 OpenAI(GPT-4o-mini)로 번역했지만, **API 비용 없이 내 서버에서 직접 굴리려고** 번역 엔진을 오픈소스 모델로 통째로 교체했습니다.
지금은 야놀자가 공개한 번역 특화 모델 **YanoljaNEXT-Rosetta-4B** 를 [Ollama](https://ollama.com)로 셀프 호스팅합니다.
→ 토큰 비용 0원, 번역 데이터도 외부로 안 나갑니다.
...
- **왜 자체 호스팅?** GPT API는 쓸 때마다 돈이 나가고, 번역문이 외부로 전송됩니다. 무료 서버(Oracle Cloud)에 직접 올리면 비용 0 + 데이터가 내 서버 밖으로 안 나갑니다.
- **왜 이 모델?** 번역 전용으로 튜닝됐고(한국어 품질 좋음), Gemma 라이선스라 **상업적 사용이 가능**하고, 4B라 GPU 없이 CPU에서도 돌아갈 만큼 가볍습니다. (참고로 NLLB·Tower 계열은 비상업 라이선스라 제외)
...
- **말투(톤) 지정** — 기본은 비즈니스 존댓말. 캐주얼 등으로 바꿀 수 있습니다.
...
TRANSLATE_TONE=Polite and professional business tone   # 말투 (선택)
TRANSLATE_GLOSSARY=라인=LINE,결제=決済                    # 용어집 (선택)
```
